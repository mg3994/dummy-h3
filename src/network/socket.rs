...
impl UdpSocket {
    ...
    pub async fn bind<A: ToSocketAddrs>(addr: A) -> H3Result<Self> {
        ...
        #[cfg(not(target_family = "wasm"))]
        {
            ...
            let socket = std::net::UdpSocket::bind(addr).map_err(|e| NetworkError::IoError(e))?;
        ...
        #[cfg(target_family = "wasm")]
        {
            ...
            let mut socket = WasiUdpSocket::new().map_err(|e| NetworkError::IoError(e))?;

            socket.bind(addr).map_err(|e| NetworkError::IoError(e))?;
    ...
    pub fn try_clone(&self) -> H3Result<Self> {
        ...
        #[cfg(target_family = "wasm")]
        {
            ...
            Err(NetworkError::OperationNotSupported(
                "Socket cloning not supported in WASI".to_string(),
            )
            .into())
        }
    }
...
mod tests {
    ...
    async fn test_socket_connect() {
        ...
        // Test send on connected socket
        let data = b"connected data";
        let send_result = socket.send(data).await;
        assert!(send_result.is_ok());
        assert_eq!(send_result.unwrap(), data.len());

        // Note: In a real test, we would need another socket to receive the data
        // For now, we'll skip testing recv() since we don't have a real socket to receive from
    }
    ...
    async fn test_socket_clone() {
        ...
        #[cfg(not(target_family = "wasm"))]
        {
            ...
            assert_eq!(
                cloned_socket.local_addr().unwrap(),
    ...
    async fn test_socket_recv_from() {
        ...
        let (bytes_read, _from_addr) = recv_result.unwrap();
